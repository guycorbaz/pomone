...
status-variety-updated = Variété mise à jour
status-variety-deleted = Variété supprimée
variety-form-section-edit = Modifier la variété
button-save-variety = Enregistrer
button-cancel-variety-edit = Annuler
...
label-harvest-expected = Rendement attendu ({ $unit })
label-harvest-actual = Rendement réel ({ $unit })
...
# Placement + courbe de capacité (Epic 3, story 3.2)
nav-placement = Placement
title-placement = Placement
placement-hint = Assigne les successions planifiées à une planche et regarde la courbe d'occupation réagir — abri et plein champ séparés. Le débordement s'affiche en ambre avec la valeur du pic.
placement-unplaced-title = À placer
placement-beds-title = Planches
placement-curve-title = Occupation du sol
placement-empty-unplaced = Rien à placer. Génère des successions depuis une ligne de plan.
placement-empty-beds = Aucune planche. Crée des lieux d'abord.
placement-col-variety = Variété
placement-col-date = Date
placement-col-bed-meters = Mètres de planche
placement-strata-label = Strate
placement-plants-label = Plants
placement-place = Placer
placement-unplace = Annuler le placement
placement-legend-open = Plein champ
placement-legend-covered = Sous abri
placement-composition-title = Séries composant le pic
placement-composition-empty = Clique sur le pic pour voir les séries qui le composent.
placement-peak-open = Pic plein champ : { $peak } / { $cap } m
placement-peak-covered = Pic sous abri : { $peak } / { $cap } m
placement-placed = Succession placée.
placement-unplaced = Placement annulé — la succession revient à placer.
placement-nothing-to-undo = Aucun placement récent à annuler.
placement-error-plants = Saisis un nombre de plants valide (> 0).
placement-error-strata = Choisis une strate.
placement-error-generic = Placement impossible.
...
crop-map-split-area = Surface ({ $unit })
...
tooltip-planting-area = Surface occupée en { $unit } (décimales acceptées, ex. 12.5).
...
## Unités d'affichage (sélecteur Paramètres, issue #29)
settings-units-section = Unités d'affichage
settings-units-explain = Unités utilisées pour afficher et saisir les surfaces et les rendements. Les données restent stockées en m² et en kg.
