//! `PlannedPlantingRepo` implementation for MariaDB.
...
impl PlannedPlantingRepo for MariaDbRepository {
    async fn planned_planting_list_for_line(
        ...
        let rows = sqlx::query(&format!(
            "SELECT {COLUMNS} FROM planned_planting \
             WHERE crop_plan_line_id = ? ORDER BY series_index"
        ))
    ...
    async fn planned_planting_delete(&self, id: PlannedPlantingId) -> DbResult<()> {
        let result = sqlx::query("DELETE FROM planned_planting WHERE id = ?")
            .bind(id.as_uuid())
            .execute(&self.pool)
            .await?;
        if result.rows_affected() == 0 {
...
fn row_to_planned(row: sqlx::mysql::MySqlRow) -> DbResult<PlannedPlanting> {
